...
WHERE rl.objid = $P{ledgerid}
 AND rl.state = 'APPROVED'
 AND (rl.nextbilldate IS NULL OR rl.nextbilldate <= GETDATE() )
 AND rli.state = 'OPEN'  
...
SELECT 
	...
	rli.year, rli.qtr,
	CASE WHEN rli.qtr = 0 THEN rli.year ELSE (rli.year + '-' + rli.qtr) END AS period,
...
SELECT 
	...
	rli.year, rli.qtr,
	CASE 
		WHEN MIN(rli.qtr) = MAX(rli.qtr) THEN (rli.year +'-' +MIN(rli.qtr))
		ELSE (rli.year + '-' +  MIN(rli.qtr) + '' + MAX(rli.qtr))
	END AS period,
	CASE WHEN rli.qtr = 0 THEN rli.year ELSE (rli.year + '-' + rli.qtr) END AS period,
